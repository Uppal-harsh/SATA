...
const SATA_PALETTE = ["#F4C542", "#58D8D5", "#F7C844", "#72EBE8", "#E88F18"];
...
export default function FluidText(props) {
  ...
  const align =
    ALIGNMENTS[String(font?.textAlign || "").toLowerCase()] ?? "center";
  const paletteColors =
  ...
  useEffect(() => {
    ...
    const paletteAt = (phase) => {
      const p = live.current;
      const list = p.paletteColors.length ? p.paletteColors : SATA_PALETTE;
      const t = tint();
    ...
    const clickSplat = () => {
      ...
      splat(pointer.texcoordX, pointer.texcoordY, 12 * (Math.random() - 0.5), 32 * (Math.random() - 0.5), {
        r: c.r * 10,
        g: c.g * 10,
        b: c.b * 10,
      });
